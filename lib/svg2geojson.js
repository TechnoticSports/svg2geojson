	...
	for (var name in layerByName) {
		...
		const geo = {
			...
			features: layerByName[name].map(data => {
				...
				if (data.coordinates.id !== null && data.coordinates.id[0] === '_') {
					data.coordinates.id = data.coordinates.id.substr(1)
				}
				return {
					type: 'Feature',
					properties: options.debug ? { id: data.coordinates.id, svgID: data.coordinates.debugId } : null,
					geometry: data
				};
			})
		};
...
function addDebugId(geo, el, fallback) {
	geo.coordinates.id = (el.$ && el.$.id) ? el.$.id.value : null
